...
void LongDouble::print_vector() {
    for (int i = 0; i < digits.size(); i++)
        cout << digits[i];
    cout << endl;
}

void LongDouble::print_exponent() {
    cout << exponent << endl;
}

void LongDouble::print_fractional_part() {
    for (int i = exponent; i < digits.size(); i++)
        cout << digits[i];
    cout << endl;
}

string LongDouble::convert_fractional_part_to_string() {
    /*string converted_part = "";
    if (exponent < 0) {
        for (int i = 0; i < std::abs(exponent); i++)
            converted_part = '0' + converted_part;
        exponent = 0;
    }
    for (int i = exponent; i < digits.size(); i++)
        converted_part += dec_digits[digits[i]];
...
string LongDouble::convert_integer_part_to_string(){
   /* */
    if (exponent <= 0 || digits[0] == 0)
            return "0";
        if (exponent > 0) {
            string converted_part = "";
...
LongDouble LongDouble::trunc(const LongDouble &number){
    LongDouble ld(number);
    string number_int_part_STRING = ld.convert_integer_part_to_string();
    return LongDouble(number_int_part_STRING);
}
